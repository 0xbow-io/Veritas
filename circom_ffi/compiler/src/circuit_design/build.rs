...
fn initialize_wasm_producer(
    ...
    producer.main_input_list =
        main_input_list(&vcp.templates[initial_node], &producer.busid_field_info);
...
fn main_input_list(main: &TemplateInstance, buses: &FieldMap) -> InputList {
        ...
        for io in &buses[busid] {
            ...
            if let Some(value) = io.bus_id {
                ...
                for (a, s) in &accesses {
                    ...
                    let mut ios =
                        get_qualified_names(value, new_start + s * io.size, prefix, buses);
    ...
    pub fn get_main_input_list_with_qualifiers(
        buses: &FieldMap,
...
fn write_main_inputs_log_new(vcp: &VCP) {
    ...
    fn write_signal(
        ...
        if bus_id.is_some() {
            ...
            for (name, info) in fields {
                write_signal(vcp, name, &info.dimensions, info.bus_id, writer)
            }
        } else {
            let msg = format!("{} {} {}\n", name, length, 0);
            writer.write_all(msg.as_bytes()).unwrap();
    ...
    for signal in &main.wires {
        if signal.xtype() == Input {
            write_signal(vcp, signal.name(), signal.lengths(), signal.bus_id(), &mut writer)
        }
        writer.flush().unwrap();
    }
}
