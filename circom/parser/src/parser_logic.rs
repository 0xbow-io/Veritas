...
pub fn preprocess(expr: &str, program_id: ProgramID) -> Result<String, ReportCollection> {
...
pub fn parse_program(
    ...
    field: &BigInt,
) -> Result<AST, ReportCollection> {
    use lalrpop_util::ParseError::*;

    let mut errors = Vec::new();
    let preprocess = preprocess(src, program_id)?;

    let ast = lang::ParseAstParser::new()
        .parse(program_id, &mut errors, field, &preprocess)
        // TODO: is this always fatal?
        .map_err(|parse_error| match parse_error {
            InvalidToken { location } => {
                produce_generic_report(format!("{:?}", parse_error), location..location, program_id)
            }
            UnrecognizedToken { ref token, .. } => {
                produce_generic_report(format!("{:?}", parse_error), token.0..token.2, program_id)
            }
            ExtraToken { ref token } => {
                produce_generic_report(format!("{:?}", parse_error), token.0..token.2, program_id)
            }
            _ => produce_generic_report(format!("{:?}", parse_error), 0..0, program_id),
        })
        .map_err(|e| vec![e])?;
