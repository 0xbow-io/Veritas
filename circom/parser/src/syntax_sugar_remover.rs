...
pub fn programs_apply_syntax_sugar(library: &mut ProgramArchive) -> Result<(), Report> {
    if library.get_main_expression().is_anonymous_comp() {
        return Result::Err(anonymous_general_error(
            library.get_main_expression().get_meta().clone(),
            "The main component cannot contain an anonymous call  ".to_string(),
        ));
    }
    let old_templates = library.templates.clone();

    for (_name, t) in library.templates.iter_mut() {
        ...
        let (new_body, component_decs, variable_decs, mut substitutions) =
            remove_anonymous_from_statement(&old_templates, &library.repository, old_body, &None)?;
    ...
    for (_, t) in &mut library.functions {
        let old_body = t.get_body().clone();
...
pub fn apply_syntactic_sugar(program_archive: &mut ProgramArchive) -> Result<(), Report> {
    ...
    let repository = program_archive.get_repo().clone();

    for (_name, t) in &mut program_archive.templates {
        let old_body = t.get_body().clone();
        check_anonymous_components_statement(&old_body)?;
        let (new_body, component_decs, variable_decs, mut substitutions) =
            remove_anonymous_from_statement(&old_templates, &repository, old_body, &None)?;
        if let Statement::Block { meta, mut stmts } = new_body {
            let mut init_block = vec![
                build_initialization_block(meta.clone(), VariableType::Var, variable_decs),
                build_initialization_block(meta.clone(), VariableType::Component, component_decs),
    ...
    match stm {
        Statement::MultSubstitution { meta, lhe, op, rhe } => {
            let (comp_declarations, mut substitutions, new_rhe) =
                remove_anonymous_from_expression(templates, repo, rhe, var_access)?;
        ...
        Statement::IfThenElse {
            ...
            let (if_body, mut if_comp_dec, mut if_var_dec, mut if_subs) =
                remove_anonymous_from_statement(templates, repo, *if_case, var_access)?;
            ...
            if else_case.is_none() {
                ...
                let (else_body, mut else_comp_dec, mut else_var_dec, mut else_subs) =
                    remove_anonymous_from_statement(templates, repo, else_c, var_access)?;
        ...
        Statement::While { meta, cond, stmt } => {
            ...
            let (body, comp_dec, mut var_dec, mut subs) =
                remove_anonymous_from_statement(templates, repo, *stmt, &Some(var_access.clone()))?;
        ...
        Statement::InitializationBlock {
            ...
            for stmt in initializations {
                let (stmt_ok, mut comps, mut vars, mut sub) =
                    remove_anonymous_from_statement(templates, repo, stmt, var_access)?;
...
pub fn remove_anonymous_from_expression(
    ...
    match exp {
        AnonymousComp {
            ...
            for (name_signal, (operator, expr)) in inputs_to_assignments {
                ...
                let (mut declarations2, mut stmts, new_exp) =
                    remove_anonymous_from_expression(templates, repo, expr, var_access)?;
        ...
        Tuple { meta, values } => {
            ...
            for val in values {
                let result = remove_anonymous_from_expression(templates, repo, val, var_access);
                match result {
        ...
        ParallelOp { meta, rhe } => {
            if rhe.is_anonymous_comp() {
                ...
                remove_anonymous_from_expression(templates, repo, rhe2, var_access)
            } else {
