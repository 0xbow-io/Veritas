...
use program_structure::program_archive::Repository;
...
fn initialize_signals(state: &mut State, signals: Vec<Signal>) {
    for signal in signals {
        ...
        state
            .signal_to_type
            .insert(signal.name.clone(), signal.xtype);
    }
}
        ...
        match component_info {
            ...
            None => {
                ...
                state
                    ...
                    .insert(trigger.component_name.clone(), new_info);
            }
        }
    }
...
fn create_uniform_components(state: &mut State, triggers: &[Trigger], cluster: TriggerCluster) {
    ...
    } = cluster.xtype
    {
        ...
        let symbol = state
            .environment
            .get_variable(&c_info.component_name)
            .unwrap()
            .clone();

        let info_parallel_cluster = state
            .component_to_parallel
            .get(&c_info.component_name)
            .unwrap();
        ...
        let creation_instr = CreateCmpBucket {
            ...
            name_subcomponent: format!(
                ...
                c_info
                    .indexed_with
                    .iter()
                    .fold(String::new(), |acc, &num| format!(
                        "{}[{}]",
                        acc,
                        &num.to_string()
                    ))
            ...
            is_part_mixed_array_not_uniform_parallel: info_parallel_cluster
...
fn translate_if_then_else(stmt: Statement, state: &mut State, context: &Context) {
    ...
    {
        let starts_at = context
            .repository
            .get_line(meta.start, meta.get_program_id())
            .unwrap();
    ...
    if let LogCall { meta, args, .. } = stmt {
        ...
        for arglog in args {
            match arglog {
                ...
                LogArgument::LogStr(exp) => match state.string_table.get(&exp) {
                    Some(idx) => {
                        logbucket_args.push(LogBucketArg::LogStr(*idx));
                    }
                    None => {
...
fn translate_variable(
    ...
    } = expression
    {
        ...
        if tag_access.is_some() {
            translate_number(
                Expression::Number(meta.clone(), tag_access.unwrap()),
                state,
                context,
            )
        } else {
    ...
    match cmp_type {
        ...
        Uniform {
            ...
            LocationRule::Indexed {
                location: full_address,
                template_header: Some(header.clone()),
            }
        }
    }
}
...
fn check_if_possible_to_usize_single(
    // returns if it is possible to convert to usize and if it is a small usize
    ...
    use Instruction::{Compute, Value};

    match &**index {
        Value(v) if v.parse_as == ValueType::U32 => (true, v.value < 100),
        Value(v) if v.parse_as == ValueType::BigInt => {
            ...
            match new_value {
                Ok(_) => (true, new_value.unwrap() < 100),
                _ => (false, false),
            }
        }
...
fn check_if_possible_to_usize_multiple(
    // returns if all of them are usize and if the number of non small usizes is at most one
    indexing: &Vec<InstructionPointer>,
    state: &State,
) -> (bool, bool) {
...
fn convert_to_usize_single(index: InstructionPointer, state: &State) -> InstructionPointer {
    ...
    match *index {
        Value(v) if v.parse_as == ValueType::U32 => v.allocate(),
...
fn translate_call_arguments(
    ...
    let mut info = ArgData {
        argument_data: vec![],
        arguments: InstructionList::new(),
    };
    for arg in args {
        let length = arg
            .get_meta()
...
pub fn translate_code(body: Statement, code_info: CodeInfo) -> CodeOutput {
    ...
    let context = Context {
        repository: code_info.repository,
