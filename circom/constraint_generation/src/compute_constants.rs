...
struct Context<'a> {
    ...
    environment: &'a EE,
    program_library: &'a ProgramArchive,
}

pub fn manage_functions(program_library: &mut ProgramArchive, prime: &String) -> CCResult {
    let mut reports = vec![];
    let mut processed = HashMap::new();
    for (name, data) in program_library.get_functions() {
        ...
        treat_statement(&mut code, &context, &mut reports, prime);
        processed.insert(name.clone(), code);
    }
    for (k, v) in processed {
        program_library.get_mut_function_data(&k).replace_body(v);
    }
    if reports.is_empty() {
        Result::Ok(())
    } else {
...
pub fn compute_vct(
    ...
    for instance in instances {
        ...
        treat_statement(&mut instance.code, &context, &mut reports, prime);
    }
    if reports.is_empty() {
        Result::Ok(())
    } else {
...
fn treat_statement(
    ...
    if stmt.is_initialization_block() {
        treat_init_block(stmt, context, reports, prime)
    } else if stmt.is_block() {
        treat_block(stmt, context, reports, prime)
    } else if stmt.is_if_then_else() {
        treat_conditional(stmt, context, reports, prime)
    } else if stmt.is_while() {
        treat_while(stmt, context, reports, prime)
    } else if stmt.is_declaration() {
        treat_declaration(stmt, context, reports, prime)
    } else if stmt.is_substitution() {
        treat_substitution(stmt, context, reports, prime)
    } else {
    }
}
    ...
    } = stmt
    {
        for init in initializations {
            if init.is_declaration() {
                treat_declaration(init, context, reports, prime)
            }
            if init.is_substitution() {
                treat_substitution(init, context, reports, prime)
            }
        }
    } else {
        unreachable!()
...
fn treat_block(
    ...
    if let Block { stmts, .. } = stmt {
        for s in stmts {
            treat_statement(s, context, reports, prime);
        }
    } else {
        unreachable!()
    }
...
fn treat_while(
    ...
    if let While { stmt, .. } = stmt {
        treat_statement(stmt, context, reports, prime);
    } else {
        unreachable!()
    }
}
...
fn treat_conditional(
    ...
    {
        treat_statement(if_case, context, reports, prime);
...
fn treat_substitution(
    ...
    if let Substitution { rhe, .. } = stmt {
        treat_expression(rhe, context, reports, prime);
    } else {
        unreachable!()
    }
}
...
fn treat_expression(
    ...
    {
        let execution_response = treat_dimension(&dimension, context, reports, prime);
        if let Option::Some(v) = execution_response {
            **dimension = Number(meta.clone(), BigInt::from(v));
        } else {
            report_invalid_dimension(meta, reports);
        }
        treat_expression(value, context, reports, prime)
    } else {
    }
}
...
fn treat_dimension(
    ...
    if context.inside_template && !dim.is_number() {
        ...
        let program = context.program_library;
        let env = context.environment;
        let execution_result = execute_constant_expression(dim, program, env.clone(), prime);
...
fn report_invalid_dimension(meta: &Meta, reports: &mut ReportCollection) {
    ...
    report.add_primary(meta.file_location(), meta.get_program_id(), message);
    reports.push(report);
}
