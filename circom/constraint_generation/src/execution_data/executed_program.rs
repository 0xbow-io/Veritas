...
impl ExecutedProgram {
    ...
    pub fn export(mut self, mut library: ProgramArchive) -> ExportResult {
        ...
        match crate::compute_constants::manage_functions(&mut library, &self.prime) {
            Ok(_) => {
                match crate::compute_constants::compute_vct(
                    &mut temp_instances,
                    &library,
                    &self.prime,
                ) {
                    Ok(_) => {
                        ...
                        let config = VCPConfig {
                            stats: dag_stats,
                            main_id: dag.main_id(),
                            repository: std::mem::take(&mut library.repository),
