use super::environment_utils::{
    environment::{
        ...
        environment_shortcut_add_output, environment_shortcut_add_bus_input, environment_shortcut_add_bus_intermediate,
        environment_shortcut_add_bus_output, environment_shortcut_add_variable, ExecutionEnvironment,
        ExecutionEnvironmentError,
    },
    slice_types::{
        AExpressionSlice, ArithmeticExpression as ArithmeticExpressionGen, ComponentRepresentation,
        ComponentSlice, MemoryError, MemorySlice, SignalSlice, SliceCapacity, TagInfo, BusSlice, BusRepresentation,
        FoldedResult, FoldedArgument, TypeAssignmentError, TypeInvalidAccess
    },
};
...
use super::{
    ast::*, ArithmeticError, ProgramArchive, ProgramID, Report, ReportCode, ReportCollection,
};
...
impl RuntimeInformation {
    pub fn new(current_program: ProgramID, id_max: usize, prime: &String) -> RuntimeInformation {
        RuntimeInformation {
            current_program,
...
pub fn constraint_execution(
    ...
    let main_program_id = program_library.get_main_program_id();
    let mut runtime_information =
        RuntimeInformation::new(*main_program_id, program_library.id_max, prime);
    use Expression::Call;

    runtime_information.public_inputs = program_library.get_public_inputs_main_component().clone();

    let folded_value_result = if let Call { id, args, .. } = &program_library.get_main_expression()
    {
        let mut arg_values = Vec::new();
        for arg_expression in args.iter() {
            let f_arg =
                execute_expression(arg_expression, program_library, &mut runtime_information);
...
pub fn execute_constant_expression(
    ...
    let current_program = expression.get_meta().get_program_id();
    let mut runtime_information =
        RuntimeInformation::new(current_program, program_library.id_max, prime);
    runtime_information.environment = environment;
    let folded_value_result =
        execute_expression(expression, program_library, &mut runtime_information);
    match folded_value_result {
        Result::Err(_) => Result::Err(runtime_information.runtime_errors),
        Result::Ok(folded_value) => {
...
fn execute_statement(
    ...
    let res = match stmt {
            ...
            match xtype {
                ...
                _ => {
                    ...
                    for dimension in dimensions.iter() {
                        let f_dimensions = execute_expression(dimension, program_library, runtime)?;
                        arithmetic_values.push(safe_unwrap_to_single_arithmetic_expression(
                            f_dimensions,
                            line!(),
                        ));
                    }
        ...
        Substitution {
            ...
            let access_information = 
                if ExecutionEnvironment::has_bus(&runtime.environment, var) || ExecutionEnvironment::has_component(&runtime.environment, var){
                    let access_bus = treat_accessing_bus(meta, access, program_library, runtime, flags)?; //note: treat_accessing_bus fn is new, decide if you want to remove flags. Old impl (https://github.com/0xbow-io/Veritas/blob/06174018a36eb203591e78d161db4c60657794e5/circom/constraint_generation/src/execute.rs#L319)
                    TypesAccess{bus_access: Some(access_bus), other_access: None}
                } else{
                    let access_other = treat_accessing(meta, access, program_library, runtime)?;
                    TypesAccess{bus_access: None, other_access: Some(access_other)}
                };

            let r_folded = execute_expression(rhe, program_library, runtime)?;
            ...
            let possible_constraint = perform_assign(
                ...
                *op, 
                ...
                runtime, 
                program_library, 
                //note: perform_assign, you removed flags (https://github.com/iden3/circom/blob/9fd40a34f42912ee52230f8b6a114d78f6df1a48/constraint_generation/src/execute.rs#L377)
            )?;
            if let Option::Some(node) = actual_node {
                if *op == AssignOp::AssignConstraintSignal || (*op == AssignOp::AssignSignal) {
                    ...
                    if !needs_double_arrow.is_empty() {
        ...
        ConstraintEquality { meta, lhe, rhe, .. } => {
            ...
            let f_left = execute_expression(lhe, program_library, runtime)?;
            let f_right = execute_expression(rhe, program_library, runtime)?;
            ...
            for i in 0..arith_left.len(){
                ...
                let possible_non_quadratic =
                    AExpr::sub(&value_left, &value_right, &runtime.constants.get_p());
        ...
        Return { value, .. } => {
            let mut f_return = execute_expression(value, program_library, runtime)?;
        ...
        LogCall { args, .. } => {
            ...
            for arglog in args {
                if let LogArgument::LogExp(arg) = arglog {
                    let f_result = execute_expression(arg, program_library, runtime)?;
        ...
        UnderscoreSubstitution { meta, rhe, op } => {
            let f_result = execute_expression(rhe, program_library, runtime)?;
...
fn execute_bus_statement(
    ...
    let _res = match stmt {
        ...
        Declaration { meta, xtype, name, dimensions, .. } => {
            ...
            for dimension in dimensions.iter() {
                let f_dimensions = 
                    execute_expression(dimension, program_library, runtime)?;
                    arithmetic_values
        ...
        Substitution { meta, var, access, op, rhe, .. } => {
            ...
            let access_information = 
                if ExecutionEnvironment::has_bus(&runtime.environment, var) || ExecutionEnvironment::has_component(&runtime.environment, var){
                    let access_bus = treat_accessing_bus(meta, access, program_library, runtime, flags)?;
                    TypesAccess{bus_access: Some(access_bus), other_access: None}
                } else{
                    let access_other = treat_accessing(meta, access, program_library, runtime)?;
                    TypesAccess{bus_access: None, other_access: Some(access_other)}
        ...
        Block { stmts, .. } => {
            execute_sequence_of_bus_statements(stmts, program_library, runtime, actual_node, flags)?;
        }
        _ => unreachable!(),
    };
    Result::Ok(())
    ...
    let res = match expr {
        ...
        Variable {
            ...
            if ExecutionEnvironment::has_signal(&runtime.environment, name) {
                execute_signal(meta, name, access, program_library, runtime)?
            } else if ExecutionEnvironment::has_component(&runtime.environment, name) {
                execute_component(meta, name, access, program_library, runtime)?
            } else if ExecutionEnvironment::has_variable(&runtime.environment, name) {
                execute_variable(meta, name, access, program_library, runtime)?
            } else if ExecutionEnvironment::has_bus(&runtime.environment, name){
                execute_bus(meta, name, access, program_library, runtime, flags)?
            }
            else {
                unreachable!();
            }
        ...
        ArrayInLine { meta, values, .. } => {
            ...
            for value in values.iter() {
                let f_value = execute_expression(value, program_library, runtime)?;
        ...
        UniformArray {
            ...
            let f_dimension = execute_expression(dimension, program_library, runtime)?;
            let arithmetic_dimension =
                safe_unwrap_to_single_arithmetic_expression(f_dimension, line!());
        ...
        InfixOp {
            ...
            let l_fold = execute_expression(lhe, program_library, runtime)?;
            let r_fold = execute_expression(rhe, program_library, runtime)?;
        ...
        PrefixOp { prefix_op, rhe, .. } => {
            let folded_value = execute_expression(rhe, program_library, runtime)?;
        ...
        InlineSwitchOp {
            ...
            let f_cond = execute_expression(cond, program_library, runtime)?;
            ...
            if let Option::Some(bool_cond) = possible_bool_cond {
                if bool_cond {
                    execute_expression(if_true, program_library, runtime)?
                } else {
                    execute_expression(if_false, program_library, runtime)?
                }
            } else {
        ...
        Call { id, args, meta, .. } => {
            let (value, can_simplify) = execute_call(id, meta, args, program_library, runtime)?;
            can_be_simplified = can_simplify;
            value
        }
        BusCall{id, args, ..} =>{
            let value = execute_bus_call_complete(id, args, program_library, runtime, flags)?;
            value
...
fn execute_call(
    ...
    let is_template = program_library.contains_template(id);

    for arg_expression in args.iter() {
        let f_arg = execute_expression(arg_expression, program_library, runtime)?;
    ...
    if program_library.contains_function(id) {
        // in this case we execute
        let new_environment = prepare_environment_for_call(id, &arg_values, program_library);
        ...
        let new_program_id = program_library.get_function_data(id).get_program_id();
        let previous_id = std::mem::replace(&mut runtime.current_program, new_program_id);

        runtime.call_trace.push(id.clone());
        let folded_result = execute_function_call(id, program_library, runtime)?;

        runtime.environment = previous_environment;
        runtime.current_program = previous_id;
        ...
        let folded_result = preexecute_template_call(id, &arg_values, program_library, runtime)?;
...
fn execute_template_call_complete(
    ...
    if program_library.contains_template(id) {
        // in this case we execute
        let new_environment = prepare_environment_for_call(id, &arg_values, program_library);
        let previous_environment = std::mem::replace(&mut runtime.environment, new_environment);
        let previous_block_type = std::mem::replace(&mut runtime.block_type, BlockType::Known);
        ...
        let new_program_id = program_library.get_template_data(id).get_program_id();
        let previous_id = std::mem::replace(&mut runtime.current_program, new_program_id);

        runtime.call_trace.push(id.clone());
        let folded_result = execute_template_call(id, arg_values, tags, program_library, runtime)?;
...
fn execute_bus_call_complete(
    ...
    if program_library.contains_bus(id){ // in this case we execute
        let new_environment = prepare_environment_for_call(id, &arg_values, program_library);
        let previous_environment = std::mem::replace(&mut runtime.environment, new_environment);
        let previous_block_type = std::mem::replace(&mut runtime.block_type, BlockType::Known);
        let previous_anonymous_components = std::mem::replace(&mut runtime.anonymous_components, AnonymousComponentsInfo::new());

        let new_file_id = program_library.get_bus_data(id).get_file_id();
        let previous_id = std::mem::replace(&mut runtime.current_file, new_file_id);

        runtime.call_trace.push(id.clone());
        let folded_result = execute_bus_call(id, arg_values, program_library, runtime, flags)?;
...
fn execute_conditional_statement(
    ...
    let f_cond = execute_expression(condition, program_library, runtime)?;
    let ae_cond = safe_unwrap_to_single_arithmetic_expression(f_cond, line!());
    ...
    if let Some(cond_bool_value) = possible_cond_bool_value {
        let (ret_value, can_simplify) = match false_case {
            Option::Some(else_stmt) if !cond_bool_value => {
                execute_statement(else_stmt, program_library, runtime, actual_node)?
            }
            Option::None if !cond_bool_value => (None, true),
            _ => execute_statement(true_case, program_library, runtime, actual_node)?,
        };
        ...
        let (mut ret_value, mut can_simplify) =
            execute_statement(true_case, program_library, runtime, actual_node)?;
...
fn execute_sequence_of_statements(
    ...
    for stmt in stmts.iter() {
        let (f_value, can_simplify) =
            execute_statement(stmt, program_library, runtime, actual_node)?;
        can_be_simplified &= can_simplify;
        if f_value.is_some() {
    ...
    if is_complete_template {
        execute_delayed_declarations(program_library, runtime, actual_node)?;
    }
...
fn execute_sequence_of_bus_statements(
    ...
    for stmt in stmts.iter() {
        execute_bus_statement(stmt, program_library, runtime, actual_node, flags)?;
    }
    Result::Ok(())
}
...
fn execute_variable(
    ...
    let access_information = treat_accessing(meta, access, program_library, runtime)?;
...
fn execute_bus(
    ...
    let access_information = treat_accessing_bus(meta, access, program_library, runtime, flags)?;
    if access_information.undefined {
        let arithmetic_slice = Option::Some(AExpressionSlice::new(&AExpr::NonQuadratic));
...
fn execute_component(
    ...
    let access_information = treat_accessing_bus(meta, access, program_library, runtime)?;
    if access_information.undefined {
        let arithmetic_slice = Option::Some(AExpressionSlice::new(&AExpr::NonQuadratic));
        return Result::Ok(FoldedValue {
            arithmetic_slice,
...
fn prepare_environment_for_call(
    ...
    let functions = program_library.get_function_names();
    let templates = program_library.get_template_names();

    let arg_names = if functions.contains(id) {
        program_library.get_function_data(id).get_name_of_params()
    } else if templates.contains(id){
        program_library.get_template_data(id).get_name_of_params()
    } else {
        // case bus
        program_library.get_bus_data(id).get_name_of_params()
    };
...
fn execute_function_call(
    ...
    let function_body = program_library.get_function_data(id).get_body_as_vec();
...
fn execute_template_call(
    ...
    let is_parallel = program_library.get_template_data(id).is_parallel();
    let is_custom_gate = program_library.get_template_data(id).is_custom_gate();
    let args_names = program_library.get_template_data(id).get_name_of_params();
    let template_body = program_library.get_template_data(id).get_body_as_vec();
    ...
    let node_pointer = if let Option::Some(pointer) = existent_node {
        ...
        let analysis =
            std::mem::replace(&mut runtime.analysis, Analysis::new(program_library.id_max));
        let code = program_library.get_template_data(id).get_body().clone();
...
fn preexecute_template_call(
    ...
    debug_assert!(runtime.block_type == BlockType::Known);
    let inputs = program_library.get_template_data(id).get_inputs();
    let outputs = program_library.get_template_data(id).get_outputs();
...
fn execute_bus_call(
    ...
    let args_names = program_library.get_bus_data(id).get_name_of_params();
    let bus_body = program_library.get_bus_data(id).get_body_as_vec();
    let mut args_to_values = BTreeMap::new();
    debug_assert_eq!(args_names.len(), parameter_values.len());
    let mut instantiation_name = format!("{}(", id);
    let mut not_empty_name = false;
    ...
    loop {
        ...
        match &access[act] {
            Access::ArrayAccess(index) => {
                let index_fold = execute_expression(index, program_library, runtime)?;
...
fn treat_accessing(
    ...
    let (ae_before_signal, signal_name, signal_index) =
        treat_indexing(0, access, program_library, runtime)?;
    let (ae_after_signal, tag_name, _tag_index) =
        treat_indexing(signal_index + 1, access, program_library, runtime)?;
    treat_result_with_memory_error(
...
fn treat_accessing_bus(
    ...
    program_library: &ProgramArchive,
    runtime: &mut RuntimeInformation,
    flags: FlagsExecution //note: remove flags just like you did for execute_expression?
    ...
    fn treat_accessing_bus_index(
        ...
        let (ae_before_signal, field_access, signal_index) =
            treat_indexing(index, access, program_library, runtime)?;
        ...
        let mut remaining_access = if signal_index < access.len(){
            Some(Box::new(
                treat_accessing_bus_index(signal_index + 1, meta, access, program_library, runtime, flags)?)
            )
        } else{
            None
        };
