...
use super::program_archive::{ProgramID, Repository};
...
impl ProgramArchive {
    pub fn new(
        repository: Repository,
        program_id_main: ProgramID,
        main_component: MainComponent,
        program_contents: Contents,
        custom_gates: bool,
    ) -> Result<ProgramArchive, (Repository, Vec<Report>)> {
        ...
        initial_template_call.fill(program_id_main, &mut fresh_id);
        if reports.is_empty() {
            Ok(ProgramArchive {
                id_max: fresh_id,
                program_id_main,
            ...
            Err((repository, reports))
        }
    }
    //program_id_main
    pub fn get_program_id_main(&self) -> &ProgramID {
