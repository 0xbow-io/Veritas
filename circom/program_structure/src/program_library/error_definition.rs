...
use super::program_archive::{ProgramID, ProgramLocation, Repository};
use codespan_reporting::diagnostic::{Diagnostic, Label};
use codespan_reporting::term;

pub type ReportCollection = Vec<Report>;
pub type DiagnosticCode = String;
type ReportLabel = Label<ProgramID>;
...
impl Report {
    ...
    pub fn print_reports(reports: &[Report], repository: &Repository) {
        use codespan_reporting::term::termcolor::{ColorChoice, StandardStream};
        let writer = StandardStream::stderr(ColorChoice::Always);
        let config = term::Config::default();
        let mut diagnostics = Vec::new();
        let code_store = repository.get_store();
        for report in reports.iter() {
            diagnostics.push(report.to_diagnostic());
        }
        for diagnostic in diagnostics.iter() {
            let print_result = term::emit(&mut writer.lock(), &config, code_store, &diagnostic);
    ...
    pub fn add_primary(
        ...
        let label = ReportLabel::primary(program_id, location).with_message(message);
    ...
    pub fn add_secondary(
        ...
        let mut label = ReportLabel::secondary(program_id, location);
    ...
    pub fn to_diagnostic(&self) -> Diagnostic<ProgramID> {
