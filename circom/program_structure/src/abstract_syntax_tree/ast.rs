use crate::{
    ...
    program_archive::{ProgramID, ProgramLocation},
};
...
#[derive(Clone)]
pub enum Pragma {
    Version(Meta, ProgramID, Version),
    CustomGates(Meta, ProgramID),
    Unrecognized,
}

pub trait FillMeta {
    fn fill(&mut self, program_id: usize, elem_id: &mut usize);
}
...
impl Meta {
    pub fn new(start: usize, end: usize) -> Meta {
        Meta {
            ...
            elem_id: 0,
            location: start..end,
            program_id: Option::None,
            component_inference: None,
            type_knowledge: TypeKnowledge::default(),
            memory_knowledge: MemoryKnowledge::default(),
        }
    }
    pub fn change_location(&mut self, location: ProgramLocation, program_id: Option<usize>) {
    ...
    pub fn get_program_id(&self) -> usize {
        if let Option::Some(id) = self.program_id {
            id
        } else {
    ...
    pub fn file_location(&self) -> ProgramLocation {
        self.location.clone()
    }
    pub fn set_program_id(&mut self, program_id: usize) {
        self.program_id = Option::Some(program_id);
    }
}
...
impl AST {
    pub fn new(
        ...
        for p in pragmas {
            match p {
                // TODO: don't panic
                Pragma::Version(location, program_id, ver) => match compiler_version {
                ...
                Pragma::CustomGates(location, program_id) => match custom_gates {
                    Some(_) => reports.push(produce_report(
                        ReportCode::MultiplePragma,
                        location.start..location.end,
                        program_id,
...
#[derive(Serialize, Deserialize, Copy, Clone, PartialOrd, PartialEq, Ord, Eq)]
...
pub fn produce_report(
    ...
    let report = match error_code {
        UnclosedComment => {
            ...
            report.add_primary(location, program_id, "Comment starts here".to_string());
            report
        }
        ...
        MissingSemicolon => {
            ...
            report.add_primary(
                location,
                program_id,
                "A semicolon is needed here".to_string(),
            );
            report
        }
        UnrecognizedInclude => {
            ...
            report.add_primary(location, program_id, "this argument".to_string());
            report
        }
        UnrecognizedPragma => {
            let mut report = Report::error(
        ...
        IllegalExpression => {
            ...
            report.add_primary(location, program_id, "here".to_string());
            report
        }
        MultiplePragma => {
            let mut report = Report::error(
...
pub fn produce_program_version_warning_report(program: usize, version: Version) -> Report {
    let mut r = Report::warning(
        format!(
            "Programs {} does not include pragma version. Assuming pragma version {:?}",
            program, version
        ),
        ReportCode::NoCompilerVersionWarning,
    );
    r.add_note(format!("At the beginning of file {}, you should add the directive \"pragma circom <Version>\", to indicate which compiler version you are using.",program));
    r
}
...
pub fn produce_program_compiler_version_report(
    program_id: usize,
    required_version: Version,
    version: Version,
) -> Report {
    let report = Report::error(
        format!("Program {} requires pragma version {:?} that is not supported by the compiler (version {:?})", program_id, required_version, version ),
...
pub fn anonymous_inside_condition_error(meta: Meta) -> Report {
    ...
    let program_id = meta.get_program_id().clone();
    report.add_primary(
        meta.location,
        program_id,
