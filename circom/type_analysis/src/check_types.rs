...
pub fn check_types(
    program_library: &mut ProgramArchive,
) -> Result<ReportCollection, ReportCollection> {
    let mut errors = ReportCollection::new();
    let mut warnings = ReportCollection::new();

    // Structural analyses
    program_level_analyses(program_library, &mut errors);
    if !errors.is_empty() {
        return Result::Err(errors);
    }

    template_level_analyses(program_library, &mut errors);
    if !errors.is_empty() {
        return Result::Err(errors);
    }

    function_level_analyses(program_library, &mut errors);
    if !errors.is_empty() {
        return Result::Err(errors);
    }

    // Decorators
    template_level_decorators(program_library, &mut errors);
    if !errors.is_empty() {
        return Result::Err(errors);
    }

    function_level_decorators(program_library, &mut errors);
    if !errors.is_empty() {
        return Result::Err(errors);
    }

    // Type analysis
    let typing_result = type_check(program_library);
    match typing_result {
        ...
        Ok(info) => {
            for name in program_library.get_function_names().clone() {
                if !info.reached.contains(&name) {
                    program_library.remove_function(&name)
                }
            }
            for name in program_library.get_template_names().clone() {
                if !info.reached.contains(&name) {
                    program_library.remove_template(&name)
                }
            }
        }
    }
    ...
    // Semantics analyses
    semantic_analyses(program_library, &mut errors, &mut warnings);
...
fn program_level_analyses(program_library: &ProgramArchive, reports: &mut ReportCollection) {
    let symbols_in_body_well_defined_result = check_naming_correctness(program_library);
...
fn template_level_analyses(program_library: &ProgramArchive, reports: &mut ReportCollection) {
    for template_data in program_library.get_templates().values() {
...
fn template_level_decorators(
    ...
    component_type_inference::inference(program_library);
    for template_data in program_library.get_mut_templates().values_mut() {
        type_reduction::reduce_template(template_data);
    }
}

fn function_level_analyses(program_library: &ProgramArchive, reports: &mut ReportCollection) {
    let function_names = program_library.get_function_names();
    for function_data in program_library.get_functions().values() {
...
fn function_level_decorators(program_library: &mut ProgramArchive, reports: &mut ReportCollection) {
    for function_data in program_library.get_mut_functions().values_mut() {
...
fn semantic_analyses(
    ...
    for template_name in program_library.get_template_names().iter() {
        if let Result::Err(mut unknown_known_report) =
            unknown_known_analysis(template_name, program_library)
        {
        ...
        if program_library
            ...
            .is_custom_gate()
        {
            let body = program_library.get_template_data(template_name).get_body();
