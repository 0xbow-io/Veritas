...
fn analyze(stmt: &Statement, entry_information: EntryInformation) -> ExitInformation {
    ...
    let program_id = entry_information.program_id;
    ...
    match stmt {
        ...
        Block { stmts, .. } => {
            ...
            let (nc, tags, ns, nr, ne, nm) =
                iterate_statements(stmts, reports, environment, program_id);
        ...
        InitializationBlock {
            ...
            let (nc, tags, ns, nr, ne, nm) =
                iterate_statements(initializations, reports, environment, program_id);
            constraints_declared = nc;
            reports = nr;
            environment = ne;
            modified_variables = nm;
...
fn check_modified(
    ...
    for v in modified_variables {
        if initial_state.has_variable(v) && final_state.has_variable(v) {
            ...
            if *t_ini != *t_fin {
                if t_fin.0 == Tag::Unknown {
                    // in other case we can enter in loops
                    modified = true;
                }
