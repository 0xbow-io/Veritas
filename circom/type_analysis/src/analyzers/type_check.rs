...
use program_structure::program_archive::{generate_program_location, ProgramArchive, ProgramID};
...
pub fn type_check(program_library: &ProgramArchive) -> Result<OutInfo, ReportCollection> {
    let mut analysis_information = AnalysisInformation {
        reached: HashSet::new(),
        program_id: *program_library.get_main_program_id(),
        reports: ReportCollection::new(),
        registered_calls: CallRegister::new(),
        environment: TypingEnvironment::new(),
        return_type: Option::None,
    };
    let initial_expression = program_library.get_main_expression();
...
fn check_main_has_tags(initial_expression: &Expression, program_library: &ProgramArchive, reports: &mut ReportCollection) {    if let Call { id, .. } = initial_expression {
        if program_library.contains_template(id) {
            ...
            for (_name,info) in inputs {
                if !info.get_tags().is_empty(){
                    ...
                    if check_bus_contains_tag_recursive(bus_name, program_library){
                        add_report(
                            ReportCode::MainComponentWithTags,
                            initial_expression.get_meta(),
                            reports,
...
fn check_bus_contains_tag_recursive(bus_name: String, program_library: &ProgramArchive) -> bool {
    let bus_data = program_library.get_bus_data(&bus_name);
...
fn type_statement(
    ...
    match statement {
        InitializationBlock {
            initializations, ..
        } => treat_sequence_of_statements(initializations, program_library, analysis_information),
        Declaration {
            xtype,
            name,
            dimensions,
            ...
            let typing_response =
                type_array_of_expressions(dimensions, program_library, analysis_information);
        ...
        Substitution {
            ...
            let rhe_response = type_expression(rhe, program_library, analysis_information);
            ...
            let access_information_result =
                treat_access(access, program_library, analysis_information);
                ...
                | (SymbolInformation::Tag, AssignOp::AssignVar) => {
                    ...
                    if analysis_information.environment.has_component(var){
                        ...
                        if comp.is_some() && access_information.1.is_some() && access_information.1.as_ref().unwrap().len() > 0 {
                            ...
                            let output = program_library.get_template_data(&template_name).get_output_info(&first_access);
                            if output.is_some() {
        ...
        ConstraintEquality { lhe, rhe, .. } => {
            let lhe_response = type_expression(lhe, program_library, analysis_information);
            let rhe_response = type_expression(rhe, program_library, analysis_information);
        ...
        LogCall { args, meta } => {
            for arglog in args {
                if let LogArgument::LogExp(arg) = arglog {
                    let arg_response = type_expression(&arg, program_library, analysis_information);
        ...
        Return { value, meta } => {
            ...
            let value_response = type_expression(value, program_library, analysis_information);
        ...
        IfThenElse {
            ...
            let cond_response = type_expression(cond, program_library, analysis_information);
            type_statement(if_case, program_library, analysis_information);
            if let Option::Some(else_stmt) = else_case {
                type_statement(else_stmt, program_library, analysis_information);
            }
        ...
        While { cond, stmt, .. } => {
            let cond_response = type_expression(cond, program_library, analysis_information);
            type_statement(stmt, program_library, analysis_information);
            let cond_type = if let Result::Ok(t) = cond_response {
                t
            } else {
                return;
...
fn type_expression(
    ...
    match expression {
        ...
        ArrayInLine { meta, values } => {
            let values_types =
                type_array_of_expressions(values, program_library, analysis_information)?;
        ...
        UniformArray {
            ...
            let value_type = type_expression(value, program_library, analysis_information)?;
            if value_type.is_template() {
                add_report(
                    ReportCode::InvalidArrayType,
                    meta,
            ...
            let dim_type = type_expression(dimension, program_library, analysis_information)?;
            if dim_type.is_template() {
                add_report(
                    ReportCode::InvalidArrayType,
                    expression.get_meta(),
        ...
        PrefixOp { rhe, .. } => {
            let rhe_type = type_expression(rhe, program_library, analysis_information)?;
            if rhe_type.is_template() || rhe_type.is_bus() || rhe_type.dim() > 0 {
                add_report_and_end(
        ...
        InlineSwitchOp {
            ...
            let cond_response = type_expression(cond, program_library, analysis_information);
            let if_true_response = type_expression(if_true, program_library, analysis_information);
            let if_false_response = type_expression(if_false, program_library, analysis_information);
        ...
        Call { id, args, meta } => {
            ...
            let typing_response =
                type_array_of_expressions(args, program_library, analysis_information);
            if program_library.contains_template(id) && typing_response.is_err() {
            ...
            if program_library.contains_template(id) && success.is_err() {
                return Result::Ok(FoldedType::template(id));
            }
            success?;
            let previous_program_id = analysis_information.program_id;
            analysis_information.program_id = if program_library.contains_function(id) {
                program_library.get_function_data(id).get_program_id()
            } else {
                program_library.get_template_data(id).get_program_id()
            };
            ...
            let returned_type = if program_library.contains_function(id) {
                type_function(
                    id,
                    &concrete_types,
                    meta,
                ...
                let r_val =
                    type_template(id, &concrete_types, analysis_information, program_library);
                Result::Ok(FoldedType::template(&r_val))
            };
            analysis_information.environment = previous_environment;
            analysis_information.program_id = previous_program_id;
        ...
        BusCall { meta, id, args } => {
            ...
            let typing_response =
                type_array_of_expressions(args, program_library, analysis_information);
            if program_library.contains_bus(id) && typing_response.is_err() {
                return Result::Ok(FoldedType::bus(id,program_library.get_bus_data(id).get_fields().len()));
            }
            let arg_types = typing_response?;
            let mut concrete_types = Vec::new();
            let mut success = Result::Ok(());
            ...
            if program_library.contains_bus(id) && success.is_err() {
                return Result::Ok(FoldedType::bus(id,program_library.get_bus_data(id).get_fields().len()));
            }
            success?;
            let previous_program_id = analysis_information.program_id;
            analysis_information.program_id = program_library.get_bus_data(id).get_program_id();
            ...
            let previous_environment =
                std::mem::replace(&mut analysis_information.environment, new_environment);
            let returned_type = type_bus(id, &concrete_types, analysis_information, program_library);
            analysis_information.environment = previous_environment;
            analysis_information.program_id = previous_program_id;
            let folded_value = returned_type?;
            Result::Ok(folded_value)
...
fn treat_access(
    ...
    for access in accesses {
        match access {
            ArrayAccess(index) => {
                let index_response = type_expression(&index, program_library, analysis_information);
...
fn apply_access_to_symbol(
    ...
    //note: was originally current_template but adjusted to current_template_or_bus. Dont think any issues should arise from this.
    let (current_template_or_bus, mut current_dim, possible_tags) = if environment.has_component(symbol) {
        let (temp, dim) = environment
    ...
    if let Option::Some(buses_and_signals) = access_information.1{
        ...
        let (mut kind, mut tags) = if environment.has_component(symbol){ 
            ...
            let input = program_library.get_template_data(&template_name).get_input_info(&accessed_element);
            let output = program_library.get_template_data(&template_name).get_output_info(&accessed_element);
        ...
        while pos < buses_and_signals.len() {
            ...
            if kind == WireType::Signal {
                    ...
                    let field = program_library.get_bus_data(&b_name).get_field_info(&accessed_element);
                    match field {
                        Some(wire) => {
...
fn type_array_of_expressions(
    ...
    for expression in expressions {
        let typing_result = type_expression(expression, program_library, analysis_information);
...
fn prepare_environment_for_call(
    ...
    let args_names = if program_library.contains_function(call_id) {
        program_library
            .get_function_data(call_id)
            .get_name_of_params()
    } else if program_library.contains_template(call_id) {
        program_library
            .get_template_data(call_id)
            .get_name_of_params()
    } else {
        program_library.get_bus_data(call_id).get_name_of_params()
    };
...
fn type_template(
    ...
    debug_assert!(program_library.contains_template(call_id));
        ...
        .is_none()
    {
        ...
        let stmts = program_library.get_template_data(call_id).get_body_as_vec();
        treat_sequence_of_statements(stmts, program_library, analysis_information);
    }
    call_id.to_string()
}

fn type_bus(id: &str, args_dims: &[ArithmeticType], analysis_information: &mut AnalysisInformation, program_library: &ProgramArchive) -> Result<FoldedType,()> {
    debug_assert!(program_library.contains_bus(id));
    if analysis_information.registered_calls.get_instance(id, args_dims).is_none() {
        ...
        let stmts = program_library.get_bus_data(id).get_body_as_vec();
        treat_sequence_of_statements(stmts, program_library, analysis_information);
    }
...
fn type_function(
    ...
    debug_assert!(program_library.contains_function(call_id));
    if let Option::Some(instance) = analysis_information
        .registered_calls
        .get_instance(call_id, args_dims)
    {
        return Result::Ok(*instance.returns());
    }
    let mut given_type = type_given_function(call_id, program_library.get_functions(), args_dims);
    ...
    let stmts = program_library.get_function_data(call_id).get_body_as_vec();
...
fn add_report(error_code: ReportCode, meta: &Meta, reports: &mut ReportCollection) {
    use ReportCode::*;
    let program_id = meta.get_program_id();
