...
use program_structure::program_archive::generate_program_location;
...
fn analyse_statement(
    ...
    use Statement::*;
    let program_id = stmt.get_meta().get_program_id();
    match stmt {
        ...
        InitializationBlock {
            ...
            if let VariableType::Signal(..) = xtype {
                ...
                let location = generate_program_location(meta.get_start(), meta.get_end());
                report.add_primary(
                    location,
                    program_id,
                    "Declaring template element".to_string(),
                );
                reports.push(report);
                return;
            }
        ...
        Substitution {
            ...
            if op.is_signal_operator() {
                ...
                let location = generate_program_location(meta.get_start(), meta.get_end());
                report.add_primary(location, program_id, "Template operator found".to_string());
                reports.push(report);
            }
...
fn analyse_expression(
    ...
    use Expression::*;
    let program_id = expr.get_meta().get_program_id();
    match expr {
        InfixOp { lhe, rhe, .. } => {
            analyse_expression(lhe, function_names, reports);
            analyse_expression(rhe, function_names, reports);
