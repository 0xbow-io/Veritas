...
pub fn inference(program_library: &mut ProgramArchive) {
    let mut template_to_inference = HashMap::new();
    for (name, data) in &program_library.templates {
        let mut analysis = PathInformation {
            components: HashSet::new(),
            environment: HashMap::new(),
        };
        infer_component_types(data.get_body(), &program_library.templates, &mut analysis);
        template_to_inference.insert(name.clone(), analysis.environment);
    }

    for (name, mut inference) in template_to_inference {
        let body = program_library.get_mut_template_data(&name).get_mut_body();
...
fn into_template_inference(expr: &Expression, templates: &TemplateInfo) -> Option<String> {
    ...
    match expr {
        ...
        ParallelOp { rhe, .. } => into_template_inference(rhe, templates),
        _ => None,
    }
}
